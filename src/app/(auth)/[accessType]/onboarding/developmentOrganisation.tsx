...
const DevelopmentOrganisation = forwardRef<{ submit: () => void; isLoading: boolean }, Props>(
  ({ setLoading, onFinish, onSuccess, initialData, isProfile }, ref) => {
    ...
    useEffect(() => {
      if (initialData) {
        reset({
          organizationName: initialData?.investorOrganizationInfo?.organizationName,
          companyEmail: initialData?.investorOrganizationInfo?.companyEmail,
          countryHeadquarters: initialData?.investorOrganizationInfo?.countryHeadquarters,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2  gap-4">
        ...
        <div>
          ...
          <CountrySelect
            ...
            onTextChange={(_txt: any) => setValue("countryHeadquarters", _txt.target.value)}
          />
        ...
        <div>
          <div className="mb-2 font-medium flex items-center gap-2">
            ...
            {uploading.certOfIncorporation && (
              <span className="ml-2 text-xs text-gray-500 animate-pulse">Uploading...</span>
            )}
          </div>
