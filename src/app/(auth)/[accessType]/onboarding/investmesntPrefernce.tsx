...
const InvestmentPreference = forwardRef<any, InvestmentPreferenceormProps>((props, ref) => {
  ...
  useEffect(() => {
    if (props?.initialData) {
      setSelectedInvestmentTypes(props?.initialData?.investorInvestmentInfo?.investmentTypes || []);

      setSelectedRegions(props?.initialData?.investorInvestmentInfo?.targetRegions || []);

      setSelectedIndustries(props?.initialData?.investorInvestmentInfo?.targetIndustries || []);

      setSelectedBusinessStages(props?.initialData?.investorInvestmentInfo?.businessStages || []);

      setMin(props?.initialData?.investorInvestmentInfo?.investmentRange?.min || "");
      setMax(props?.initialData?.investorInvestmentInfo?.investmentRange?.max || "");
      setCurrency(props?.initialData?.investorInvestmentInfo?.investmentRange?.currency || "USD");
      reset({
  ...
  useImperativeHandle(ref, () => ({
    submit: () => {
      return new Promise((resolve) => {
        handleSubmit(
          (values) => {
            // Custom validation: min < max (only if both are provided)
            if (
              Number.isFinite(min) &&
              Number.isFinite(max) &&
              typeof min === "number" &&
  ...
  const [currentInvestmentType, setCurrentInvestmentType] = useState<string | undefined>(undefined);
  ...
  return (
    <form className="gap-4 flex flex-col" onSubmit={handleSubmit(onSubmit)}>
      <div className="grid md:grid-cols-2 w-full gap-4">
        <div>
          <label className="block mb-1 text-[10px] font-medium">Investment Type</label>
          <MultiSelect
        ...
        <div>
          <label className="block mb-1 text-[10px] font-medium">Target Regions</label>
          <MultiSelect selectedItems={selectedRegions} onValueChange={handleRegionChange}>
            <MultiSelectTrigger selectedItems={selectedRegions} onRemoveItem={handleRemoveRegion}>
        ...
        <div>
          <label className="block mb-1 text-[10px] font-medium">Target Industries</label>
          <MultiSelect selectedItems={selectedIndustries} onValueChange={handleIndustryChange}>
            <MultiSelectTrigger
        ...
        <div>
          <label className="block mb-1 text-[10px] font-medium">Business Stage*</label>
          <MultiSelect
      ...
      <div className="flex flex-col">
        <label className="block mb-1 text-[10px] font-medium">Investment Range</label>
        <div className="grid md:grid-cols-2 w-full gap-4">
          <div>
            <CurrencyAmountInput
              tag="Min"
            ...
            />
            {errors.min && (
              <span className="col-span-2 text-[10px] text-red-500">{errors.min.message}</span>
            )}
          </div>
          <div>
            <CurrencyAmountInput
            ...
            />
            {errors.max && (
              <span className="col-span-2 text-[10px] text-red-500">{errors.max.message}</span>
            )}
          </div>
        </div>
      </div>
