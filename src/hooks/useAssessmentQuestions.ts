...
export const useAssessmentQuestionsByCategory = (category: AssessmentCategory) => {
  return useQuery({
    queryKey: ["assessment-questions", category],
    queryFn: async () => {
      const res = await api.get(assessmentQuestionsRoutes.getQuestionsByCategory(category));
    ...
    queryFn: async () => {
      ...
      const results = await Promise.all(
        categories.map(async (category) => {
          ...
          return {
            category,
            questions: (res.data as AssessmentQuestion[]).sort((a, b) => a.order - b.order),
          };
        }),
      );
      return results;
