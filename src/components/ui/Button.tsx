...
const Button: React.FC<ButtonProps> = ({
  ...
  const variantClasses: Record<ButtonVariant, Record<ButtonState, string>> = {
    primary: {
      default: "bg-green text-white hover:bg-primary-green-7 focus:ring-teal-500",
    ...
    secondary: {
      default: "bg-white text-green border border-green hover:bg-gray-50 focus:ring-gray-500",
      ...
      disabled: "bg-transparent text-black-50 border border-black-50 cursor-not-allowed",
    },
    tertiary: {
      default: "bg-transparent text-black-400 border-none hover:bg-gray-100 focus:ring-gray-500",
  ...
  const buttonClasses: string =
    `${baseClasses} ${sizeClasses[size]} ${variantClasses[variant][state]} hover:scale-105 transition-all duration-300 cursor-pointer rounded-md ${className}`.trim();
  ...
  const renderIcon = (): React.ReactElement | null => {
    ...
    switch (iconPosition) {
      ...
      case "chat":
        return <img src={"/images/chatIcon.svg"} className="w-4 h-4" />;
      default:
        return null;
    }
  };
...
export type { ButtonProps, ButtonSize, ButtonState, ButtonVariant, IconPosition };
