...
export const ResourceCardSkeleton = ({ variant = 'hero' }: SkeletonProps) => {
  ...
  return (
    <motion.div
      ...
      className={`bg-[#FCFCFC] border rounded-3xl gap-[24px] items-start flex flex-col animate-pulse ${isHero
        ? 'w-sm border-[#E8E8E8] rounded-3xl'
        : 'w-[384px] border-black-50 rounded-2xl overflow-hidden'
        }`}
    >
      <div className={`w-full bg-gray-200 ${isHero ? 'h-48 rounded-t-3xl' : 'h-[284px]'
        }`}></div>
      <div className={`flex flex-col gap-4 w-full ${isHero ? 'p-3 mb-6' : 'p-6'
        }`}>
        <div className={`bg-gray-200 rounded w-3/4 ${isHero ? 'h-6' : 'h-8 mb-4'
          }`}></div>
